...
for bsi in bark:
    ...
    if np.where(loudn_freq == bsi)[0].size != 0: # loudness value for this frequency already exists
        ...
        w1 = old_div(1, np.abs(loudn_freq[j] - bsi))
        w2 = old_div(1, np.abs(loudn_freq[j + 1] - bsi))
        loudn_bark[:,i] = old_div((eq_loudness[:,j]*w1 + eq_loudness[:,j+1]*w2), (w1 + w2))
...
for i in range(n_bark_bands):
    CONST_spread[i,:] = 10**(old_div((15.81+7.5*((i-np.arange(n_bark_bands))+0.474)-17.5*(1+((i-np.arange(n_bark_bands))+0.474)**2)**0.5),10))
...
def periodogram(x,win,Fs=None,nfft=1024):
    ...
    P  = old_div(Xx*np.conjugate(Xx),U)
    ...
    if nfft % 2 != 0:
        select = np.arange(old_div((nfft+1),2))  # ODD
...
def calc_spectrogram(wavsegment,fft_window_size,fft_overlap = 0.5,real_values=True):
    ...
    n_frames = int(old_div((wavsegment.shape[0] - fft_window_size), hop_size) + 1)
...
def transform2phon(matrix):
    ...
    ifac = old_div((matrix[:,0:t] - cbv.transpose().ravel()[cbv_ind_lo]), (cbv.transpose().ravel()[cbv_ind_hi] - cbv.transpose().ravel()[cbv_ind_lo]))
...
def transform2sone(matrix):
    ...
    matrix[idx]     =  2**(old_div((matrix[idx]-40),10))    #
    matrix[not_idx] =  (old_div(matrix[not_idx],40))**2.642 # max => 438.53
    return(matrix)
...
def rp_extract( wavedata,                          # pcm (wav) signal data normalized to (-1,1)
    ...
    if ((skip_leadin_fadeout > 0) or (step_width > 1)):
        ...
        duration =  old_div(wavedata.shape[0],samplerate)
    ...
    n_segments = int(np.floor( old_div((np.floor( old_div((wavedata.shape[0] - (skip_seg*2*segment_size)), segment_size) ) - 1 ), step_width) ) + 1)
    ...
    for seg_id in range(n_segments):
        ...
        rhythm_patterns = old_div(rhythm_patterns, 256)  # why 256?
        ...
        if (extract_rp or extract_rh2) and fluctuation_strength_weighting:
            ...
            mod_freq_res  = old_div(1, (float(segment_size) / samplerate))
            ...
            fluct_curve = old_div(1, (old_div(mod_freq_axis,4) + old_div(4,mod_freq_axis)))
    ...
    if return_segment_features:
        ...
        features["timepos"] = old_div(features["segpos"], (samplerate * 1.0))
