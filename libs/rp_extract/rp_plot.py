...
def logscale_spec(spec, sr=44100, factor=20.):
    ...
    scale *= old_div((freqbins - 1), max(scale))
...
def plotstft(samples, samplerate, binsize=2 ** 10, plotpath=None, colormap="jet", ax=None, fig=None, plot_width=6,
    ...
    ims = 20. * np.log10(old_div(np.abs(sshow), 10e-6))  # amplitude to decibel
    ...
    ax.set_xticks(xlocs, ["%.02f" % l for l in old_div(((old_div(xlocs * len(samples), timebins)) + (0.5 * binsize)), samplerate)])
    ...
    #plt.clf();
    b = ["%.02f" % l for l in old_div(((old_div(xlocs * len(samples), timebins)) + (0.5 * binsize)), samplerate)]
